...
const WITNESS = join(HERE, ".mutation-check-dirty-guard-witness.tmp");
...
test("mutation-check aborts before doing anything if the working tree is dirty", { skip }, () => {
  ...
  try {
    ...
    assert.match(
      result.out,
      /\.mutation-check-dirty-guard-witness\.tmp/,
      `expected the guard's own git-status output to name the file that dirtied the tree:\n${result.out}`,
    );
  } finally {
